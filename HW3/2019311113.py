...
class NeuralNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.max_pool2d(F.relu(self.bn2(self.conv2(x))),(2,2))# 28/2=14
        x = F.relu(self.bn3(self.conv3(x)))# (14-3+1)=12
        x = F.max_pool2d(F.relu(self.bn4(self.conv4(x))),(2,2))# (12-3+1)/2=5
...
optimizer = optim.Adam(model.parameters(), lr=learning_rate)#, momentum=momentum)#,weight_decay=0.001)
#criterion = nn.MSELoss()
criterion = nn.CrossEntropyLoss()
# train model using train_image and train_label
maxAccuracy = 0
maxEpoch = 0
trainlosslist = np.zeros(epochs)
testaccuracylist = np.zeros(epochs)
...
# get predictions on test_image
    ...
    testaccuracylist[epoch] = correct
# evaluation
model.eval()
with torch.no_grad():
    x_test = x_test.float()
...
with h5py.File('acc.h5', 'w') as opt:
    opt.create_dataset('train', data=trainlosslist,compression='gzip')
    opt.create_dataset('test', data=testaccuracylist,compression='gzip')
