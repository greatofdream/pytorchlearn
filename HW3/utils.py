...
#https://docs.scipy.org/doc/scipy/reference/release.1.3.0.html?highlight=imread#scipy-interpolate-changes
def LoadData(num_classes = 50, num_samples_per_class_train = 15, num_samples_per_class_test = 5, seed = 1):
    ...
    all_labels = np.zeros(shape = (num_samples_per_class, num_classes),dtype=int)
    #all_labels = np.zeros(shape = (num_samples_per_class, num_classes,num_classes),dtype=int)
    ...
    for label,imagefile in label_images:
        ...
        #all_labels[temp_num, label,label] = 1
        temp_count[label] += 1
    ...
    #train_label = all_labels[:num_samples_per_class_train].reshape(-1,num_classes)
    #test_label  = all_labels[num_samples_per_class_train:].reshape(-1,num_classes)
